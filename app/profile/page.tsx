...
export default async function ProfilePage() {
  ...
  let purcheseDetailBooks: BookType[] = [];
  if (user) {
    ...
    purcheseDetailBooks = await Promise.all(
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="flex items-center gap-6">
        {purcheseDetailBooks.map((purcheseDetailBook: BookType) => (
          <PurchaseDetailBook
            key={purcheseDetailBook.id}
            purchaseDetailBook={purcheseDetailBook}
          />
        ))}
      </div>
    </div>
